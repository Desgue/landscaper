...
Defines the Go structs for `internal/model/request.go`. These are the only types the backend needs to parse from the incoming JSON. Fields not listed here (groups, journalEntries, viewport, uiState, zIndex, locked, notes, etc.) are irrelevant to the backend pipeline and are intentionally omitted — Go's `encoding/json` decoder ignores unknown fields silently.

All field names must match the JSON keys defined in [data-schema.md] exactly.
...
## Unused Frontend Fields (Intentionally Omitted)

These fields from the frontend schema are never read by the backend. They are decoded to `interface{}` by Go's JSON decoder and discarded — no action needed.

| Frontend field | Reason omitted |
|---|---|
| `project.version`, `exportedAt` | Not on the project object itself (top-level export fields); not needed by backend |
| `project.gridConfig` | Grid only relevant to frontend rendering |
| `project.currency` | Cost tracking is frontend-only |
| `project.viewport`, `project.uiState` | Restore-session fields; not used in image generation |
| `project.groups` | Grouping is a frontend edit concept; elements still carry `layerId` |
...
- `encoding/json` ignores unknown fields by default — no `DisallowUnknownFields()` call.
- `Element.Quantity` zero value is `0`; treat as `1` when zero (matches frontend default of `1`).
- `Element.Rotation` zero value is a valid rotation (0°) — always safe to use directly.
- `EffectiveOptions.Seed == -1` means random; pass no seed to Gemini. See gemini-client.md "## Request Construction".
- `ProjectPayload.Layers` may be empty for projects with no explicit layer data — treat all elements as on a visible layer in this case.
